...
public:
	...
	template<class Type> struct BaseObjectInfo
	{
		Type * data;
		int refCounter;
		int ptrCounter;
...
#endif
		static void addReference(Type * data)
		{
			assert(data != NULL);
			assert(data->objectInfo != NULL);
			data->objectInfo->refCounter++;
		}		
		static bool removeReference(Type * data)
		{
			assert(data);
			assert(data->objectInfo);
			BaseObjectInfo<Type> * info = data->objectInfo;
			data->objectInfo->refCounter--;
		...
		static void releaseObject(Type * data)
		{
			assert(data);
		...
		static BaseObjectInfo<Type> * allocate()
		{
#ifdef BASE_OBJECT_INFO_STATIC
			static int counter = 0;
			static ObjectInfo headers[8];
			assert(counter < 8);
			BaseObjectInfo<Type> * result = headers + counter++;
			result->flags = counter;
			return result;
#else
			return new BaseObjectInfo<Type>;
#endif
		}

		static bool releaseInfo(BaseObjectInfo<Type> * info)
		{
			info->ptrCounter--;
...
public:
	template< class Type > static Referenced::BaseObjectInfo<Type> * getObjectInfo( Type * object )
	{
		Referenced * ref = object;
		return (BaseObjectInfo<Type>*)ref->objectInfo;
	}
};
...
public:
	...
	typedef typename RefBase::BaseObjectInfo<Type> ObjectInfo;
...
private:
	my_type(const Type & data) : instance(data) 
	{
		bind();
	}
...
public:
	~Instance()	{}
	template<class Arg0> my_type(Arg0 arg0) : instance(arg0) 
	{
		bind();
	}
	template<class Arg0, class Arg1> my_type(Arg0 arg0, Arg1 arg1) : instance(arg0,arg1) 
	{
		bind();
	}	
