...
public:
	class Scoped;
	enum {maxThreat=-1};
	Log():level(0),threatLevel(0),last(NULL),fill(' '){}	
	...
	public:
		Scoped(Log &l,const char *text,int threatLevel)
			:threat(threatLevel),text(text),entry(false),root(l),prev(NULL)
		{
			root.push(this);
		}		
		~Scoped()
...
#ifdef __FUNCTION__
#define LogScope(log,level) Log::Scoped __logFunction((log),__FUNCTION__,(level))
#define LogFunction(log) LogScope(log,0)
#else
#define LogScope(log,level) (0)
#define LogFunction(log) LogScope(log,0)
#endif
